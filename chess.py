 ...
 f"  1| {fb[0][0]} |_{fb[1][0]}_| {fb[2][0]} |_{fb[3][0]}_| {fb[4][0]} |_{fb[5][0]}_| {fb[6][0]} |_{fb[7][0]}_|\n"
  "     A   B   C   D   E   F   G   H")
    ...
    def move(self, spot_from, spot_to):  # >0 success, <= 0 not success
        ...
        legal_moves = check_legal(self, spot_from, av_moves(self, spot_from))
        ...
        for spot in legal_moves:
            if spot.strip().capitalize() == spot_to.strip().capitalize():
                ...
                if isinstance(self.board[lt][nt], Piece):
                    action = 'eating'
                else:
                    action = 'moving'
                piece_type = self.board[lf][nf].type
...
def get_action(chess, move_from, move_to):
    pieces_names = ['Pawn', 'King', 'Queen', 'Tower', 'Knight', 'Bishop']
    piece = -1
    if isinstance(chess.board[letter_to_number(move_from[0])][int(move_from[1]) - 1], Piece):
        piece = chess.board[letter_to_number(move_from[0])][int(move_from[1]) - 1].type
    if isinstance(chess.board[letter_to_number(move_to[0])][int(move_to[1]) - 1], Piece):
        l = letter_to_number(move_to[0])
        action = f"eating {pieces_names[chess.board[l][int(move_to[1]) - 1].type].lower()} \n"
    else:
        action = "moving "
    return action, piece
...
def choose_transf_option(player):
    t = -1
    if player.bot:
        options = [1, 1, 1, 2, 3, 4, 1, 1, 1, 2, 3, 4, 1, 1, 1, 1, 1, 1]
...
def make_rand_move(chess, player):
    ...
    if player.is_black:
        if player.can_short_castling:
            spot_from, spot_to = "E8", "G8"
            if castling(chess, spot_from, spot_to, player):
                c = True

        if player.can_long_castling:
            spot_from, spot_to = "E8", "C8"
            if castling(chess, spot_from, spot_to, player):
                c = True
    else:
        if player.can_short_castling:
            spot_from, spot_to = "E0", "G0"
            if castling(chess, spot_from, spot_to, player):
                c = True
        if player.can_long_castling:
            spot_from, spot_to = "E0", "c0"
            if castling(chess, spot_from, spot_to, player):
                c = True
    if c:
        print(f"\nCastling from {spot_from} to {spot_to} ...\n\n...\n")
        sleep(2)
        return 0
    pieces_names = ['Pawn', 'King', 'Queen', 'Tower', 'Knight', 'Bishop']
    i = 0
    ...
    action, piece = get_action(chess, spot_from, spot_to)
    chess.move(spot_from, spot_to)
    print(f"\n{pieces_names[piece]} {action}from {spot_from} to {spot_to} ...\n\n...\n")
    pawn_transformation(chess, spot_to, player)
    sleep(1.5)
    return 0
                if l == 0:  #A
                    if n == 0:  #1
